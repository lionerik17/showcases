...
void initUniforms() {
    ...
    airportModelMatrix = glm::scale(glm::mat4(1.0f), glm::vec3(0.1f, 0.1f, 0.1f));
    //model = glm::rotate(model, glm::radians(angle), glm::vec3(0.0f, 1.0f, 0.0f));
	airportModelLoc = glGetUniformLocation(myBasicShader.shaderProgram, "model");

	airplaneModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(100.0f, 2.5f, -70.0f));
...
void renderAirplane(gps::Shader shader) {
	...
	// Set airplane model matrix
	glUniformMatrix4fv(glGetUniformLocation(shader.shaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(airplaneModelMatrix));

	// Set camera position
	glUniform3fv(glGetUniformLocation(shader.shaderProgram, "cameraPos"), 1, glm::value_ptr(myCamera.getPosition()));
